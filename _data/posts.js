...
module.exports = function() {
    return new Promise(async (resolve, reject) => {
      ...
      while (i< response.results.length) {
         ...
         var mddata = await new NotionExporter(process.env.COOKIE).getMdString(pagedata.id)
         var html = rewriteNotionImages(md.render(mddata))
         var title = pagedata.properties.Title.title[0].plain_text
      obj.push({
      title: title,
      date:moment(new Date(pagedata.properties.Date.date.start)).format("MMMM do[,] YYYY"),
      html:html.split("\n").slice(5).join("\n"),
      readTime:readingTime(mddata).text,
      slug:slugify(title),
      description:desc(html).replace(/(<([^>]+)>)/ig,""),
    
      })
            i++
        }
     
      resolve(obj.sort(function(a, b) {
